...
class NetworkAnalyzer_Hydro:
    ...
    def plot_CO2_intensity_comparison_all(self, bus):
        ...
        ref_file_path = self.base_dir / 'data' / 'co2_emissions' / 'aggregated_data' / f'co2_intensity_{weather_year}.csv'
        ...
        # Plotting
        fig, ax = plt.subplots(figsize=(self.x_length, self.y_length))
        ax.plot(df_co2_intensity.index, df_co2_intensity.values, label='PyPSA CO2 Intensity', color=self.nature_vermillion, linewidth=1)
        ax.plot(ref_co2_df_intensity.index, ref_co2_df_intensity.values, label='Reference CO2 Intensity', color=self.nature_blue, linewidth=1)
