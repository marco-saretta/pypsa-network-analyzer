...
class NetworkAnalyzer:
    ...
    def _set_directories(self, network_file: str):
        ...
        if not self.network_file_dir.exists():
            msg = f"Network file {self.network_file_dir} does not exist in data folder"
        ...
        if self.network_file_dir.suffix != ".nc":
            msg = f"Invalid network file extension: {self.network_file_dir.suffix}. Expected .nc"
    ...
    def plot_CO2_intensity_comparison_all(self, bus):
        ...
        if weather_year != "2020":
            ref_file_path = (
                self.root_dir / "data" / "co2_emissions" / "aggregated_data" / f"co2_intensity_{weather_year}.csv"
            )
        ...
        ax.set_title(rf"CO$_2$ Emissions Comparison: {bus} {weather_year}", fontsize=14, fontweight="bold")
        ax.legend(loc="best")
    ...
    def plot_total_dispatch_all_buses(self):
        ...
        # ===============================================================
        fig, ax = plt.subplots(figsize=(self.x_length, self.y_length))
        ax.barh(series.index, series.values, color=self.nature_sky_blue, edgecolor="black", linewidth=0.5)
        ...
        if len(series) > 6:
            ax.legend(wedges, series.index, title="Carrier", bbox_to_anchor=(1.05, 1), loc="upper left")
    ...
    def plot_generation_comparison_all(self, bus):
        ...
        external_file = (
            self.root_dir / "data" / "generation" / "generation_hourly_data" / f"generation_{bus}_hourly_data.csv"
        )
    ...
    def plot_EUR_country_generation_function(self):
        ...
        mapping = {
            "0 offwind-ac": "Wind Offshore",
            "1 offwind-ac": "Wind Offshore", 
            "2 offwind-ac": "Wind Offshore",
            "0 offwind-dc": "Wind Offshore",
            "1 offwind-dc": "Wind Offshore",
            "2 offwind-dc": "Wind Offshore",
            "0 onwind": "Wind Onshore",
            "1 onwind": "Wind Onshore",
            "2 onwind": "Wind Onshore",
            "0 solar": "Solar",
            "1 solar": "Solar",
            "2 solar": "Solar",
            "0 solar-hsat": "Solar",
    ...
    def plot_all_figures(self):
        ...
        for bus in self.buses:
            bus_folder = self.network_file_res_dir / bus
