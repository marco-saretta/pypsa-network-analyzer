...
class NetworkAnalyzer_Hydro:
    ...
    def compute_dispatch(self):
        ...
        phs_dispatch_t_bus = n.storage_units_t.p_dispatch.T.groupby(phs_gen_bus).sum().T
        ...
        gen_filtered_t_bus = gen_filtered_t_bus.add(phs_dispatch_t_bus, fill_value=0)
        gen_t_bus = gen_t_bus.add(hydro_dispatch_t_bus, fill_value=0)
        gen_t_bus = gen_t_bus.add(phs_dispatch_t_bus, fill_value=0)
    ...
    def extract_summary(self):
        ...
        self.capacities.to_csv(folder / "installed_capacities_MW.csv")
        # Total capacities by carrier
        self.capacities_by_carrier = self.total_capacities.groupby(self.carrier_total).sum()
    ...
    def plot_CO2_intensity_comparison_all(self, bus):
        ...
        # Retrieve the CO2 emissions time series from PyPSA network
        pypsa_co2 = self.co2_bus
        co2_df = pypsa_co2[bus]  # tCO2 per Mwh

        # # Retrieve the load time series to calculate emissions intensity
        load = self.load_df[bus]
        # Calculate CO2 intensity (emissions / load)
        df_co2_intensity = co2_df / load
        # Convert to gCO2/kWh
        df_co2_intensity = df_co2_intensity * 1000

        # Get the reference data for comparison
        ref_file_path = (
            self.base_dir / "data" / "co2_emissions" / "aggregated_data" / f"co2_intensity_{weather_year}.csv"
        )
        df_co2 = pd.read_csv(ref_file_path, parse_dates=["DateTime (UTC)"])
        df_co2 = df_co2.set_index("DateTime (UTC)")

        # filter df_co2 for the specific bus (country)
        if bus not in df_co2.columns:
            print(f"No reference CO2 data found for bus '{bus}'")
            return
        ref_co2_df_intensity = df_co2[bus]

        # Plotting
        fig, ax = plt.subplots(figsize=(self.x_length, self.y_length))
        ax.plot(
            df_co2_intensity.index,
            df_co2_intensity.values,
            label="PyPSA CO2 Intensity",
            color=self.nature_vermillion,
            linewidth=1,
        )
        ax.plot(
            ref_co2_df_intensity.index,
            ref_co2_df_intensity.values,
            label="Reference CO2 Intensity",
            color=self.nature_blue,
            linewidth=1,
        )

        ax.set_xlabel("Time")
        ax.set_ylabel("CO₂ Emissions in g/kWh")
        ax.set_title(f"CO₂ Emissions Comparison: {bus} {weather_year})")
        ax.legend(loc="best")
    ...
    def plot_generation_comparison_all(self, bus):
        ...
        phs_dispatch = self.n.storage_units_t.p_dispatch[self.phs_units.index]
        pypsa_gen = pypsa_gen.join(hydro_dispatch).join(phs_dispatch)
        ...
        external_file = (
            self.base_dir / "data" / "generation" / "generation_hourly_data" / f"generation_{bus}_hourly_data.csv"
        )
    ...
    def plot_hydro_analysis(self, bus):
        ...
        # Hydro inflow
        hydro_inflow_res = self.n.storage_units_t.inflow.loc[
            :,
            self.n.storage_units_t.inflow.columns.str.contains(bus, na=False)
            & self.n.storage_units.carrier[self.n.storage_units_t.inflow.columns].eq("hydro").values,
        ]
        hydro_inflow_phs = self.n.storage_units_t.inflow.loc[
            :,
            self.n.storage_units_t.inflow.columns.str.contains(bus, na=False)
            & self.n.storage_units.carrier[self.n.storage_units_t.inflow.columns].eq("PHS").values,
        ]
        ...
        soc_phs = self.n.storage_units_t.state_of_charge.loc[
            :,
            self.n.storage_units_t.state_of_charge.columns.str.contains(bus, na=False)
            & self.n.storage_units.carrier[self.n.storage_units_t.state_of_charge.columns].eq("PHS").values,
        ]
        soc_hydro = self.n.storage_units_t.state_of_charge.loc[
            :,
            self.n.storage_units_t.state_of_charge.columns.str.contains(bus, na=False)
            & self.n.storage_units.carrier[self.n.storage_units_t.state_of_charge.columns].eq("hydro").values,
        ]
        fig, axs = plt.subplots(1, 2, figsize=(14, 5), constrained_layout=True)
        soc_phs.sum(axis=1).plot(ax=axs[0], title="PHS State of Charge (TWh)", color=self.nature_sky_blue)
        soc_hydro.sum(axis=1).plot(
            ax=axs[1], title="Hydro Reservoir State of Charge (TWh)", color=self.nature_bluish_green
        )
        self._save_plot(fig, f"{bus}_hydro_soc", bus=bus)

        # 3. Power "p"
        phs_p = self.n.storage_units_t.p.loc[
            :,
            self.n.storage_units_t.p.columns.str.contains(bus, na=False)
            & self.n.storage_units.carrier[self.n.storage_units_t.p.columns].eq("PHS").values,
        ]
        hydro_p = self.n.storage_units_t.p.loc[
            :,
            self.n.storage_units_t.p.columns.str.contains(bus, na=False)
            & self.n.storage_units.carrier[self.n.storage_units_t.p.columns].eq("hydro").values,
        ]
        # Resample to daily
