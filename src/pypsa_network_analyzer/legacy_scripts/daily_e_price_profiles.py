...
def country_level_prices(price_df, load_df, country_mapping):
    ...
    for region, bidding_zones in country_mapping.items():
        ...
        country_price_df[region] = load_weighted_price(price_df=price_df, load_df=load_df, bidding_zones=bidding_zones)
...
country_prices = country_level_prices(price_df=e_prices_common, load_df=load_common, country_mapping=country_mapping)
...
price_sources = {"historical": e_prices, "pypsa": e_prices_pypsa}
...
for source_name, e_prices_source in price_sources.items():
    for country in countries:
        ...
        if source_name == "pypsa":
            # Create directories
            plot_dir = file_dir / "data" / "prices" / "electricity_prices_plots" / simulation / source_name / country
        elif source_name == "historical":
            # Create directories
            plot_dir = file_dir / "data" / "prices" / "electricity_prices_plots" / source_name / country
        ...
        for year in years:
            for time_filter in time_filter_dict:
                if time_filter == "year":
                    ...
                    end_date = pd.Timestamp(start_date) + pd.DateOffset(years=1) - pd.Timedelta(hours=1)
                ...
                hourly_stats = df.groupby("hour")["price"].agg(
                    **{f"p{p}": lambda x, q=p / 100: x.quantile(q) for p in percentiles}, median="median", mean="mean"
                )
                ...
                for i in range(len(percentiles) // 2):
                    ...
                    alpha = 0.1 + 0.2 * i
                    plt.fill_between(
                        hourly_stats.index, hourly_stats[lower], hourly_stats[upper], color="blue", alpha=alpha
                    )

                # Median & mean
                plt.plot(hourly_stats.index, hourly_stats["median"], color="red", linewidth=2, label="Median")
                ...
                # Legend
                gradient_handles = [Patch(facecolor="blue", alpha=0.1 + 0.2 * i) for i in range(len(percentiles) // 2)]
                gradient_labels = [
                    f"{percentiles[i]}–{percentiles[-(i + 1)]} percentile" for i in range(len(percentiles) // 2)
                ]
