...
def plot_by_country_and_aggregated(
    config, all_dfs_dict, countries, years_reference, plot_base_dir, time_period_label="full_period"
):
    """
    ...
    for country in countries:
        ...
        ax.set_title(
            f"CO₂ Intensity: {country} {period_info} - {config['title_suffix']}",
            fontsize=10,
            fontweight="bold",
            loc="left",
        )
    ...
    for dataset_name, label in zip(config["datasets"], config["labels"]):
        ...
        color = colors.get(dataset_name)
        ax.plot(df_mean.index, df_mean.values, label=label, alpha=0.8, linewidth=1.5, linestyle="-", color=color)
        plotted_any = True
    ...
    if plotted_any:
        ...
        period_info = f"({year_filter})" if year_filter else "(2020-2024)"
        ax.set_title(
            f"CO₂ Intensity: Aggregated (Average all countries) {period_info} - {config['title_suffix']}",
            fontsize=10,
            fontweight="bold",
            loc="left",
        )
...
def run_all_plots(plot_dir, all_dfs_dict, countries, years_reference):
    ...
    for config in plot_configs_full_period:
        ...
        plot_by_country_and_aggregated(
            config, all_dfs_dict, countries, years_reference, plot_dir, time_period_label="full_period"
        )
    ...
    for config in plot_configs_2022_only:
        print(f"\nProcessing: {config['name']}")
        plot_by_country_and_aggregated(
            config, all_dfs_dict, countries, years_reference, plot_dir, time_period_label="2022_only"
        )
